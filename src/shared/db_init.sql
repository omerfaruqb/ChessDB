-- Drop existing tables if they exist to ensure a clean slate (reverse order of creation)
...
CREATE TABLE Specialty (4specialty_name VARCHAR (50) PRIMARY KEY);
-- 9. CoachCertification Table (For Coach certifications)
CREATE TABLE CoachCertification (certification_name VARCHAR (50) PRIMARY KEY);
-- 10. ArbiterCertification Table (For Arbiter certifications)
CREATE TABLE ArbiterCertification (certification_name VARCHAR (50) PRIMARY KEY);
-- 11. Hall Table
CREATE TABLE Hall (
...
CREATE TABLE ChessMatch (
    ...
    start_slot INT NOT NULL CHECK (
        start_slot BETWEEN 1 AND 3
    ),
    ...
    result VARCHAR (10) NOT NULL CHECK (
        result IN (
            ’ white_wins ’,
            ’ black_w FOREIGN KEY (tournament_ID) REFERENCES Tournament(tournament_ID),
            FOREIGN KEY (hall_ID, table_ID) REFERENCES ‘ Table ‘(
                hall_ID,
                table_ID FOREIGN KEY (white_team_ID) REFERENCES Team(team_ID),
                ...
                FOREIGN KEY (assigned_arbiter_username) REFERENCES Arbiter(username) CHECK (white_player_username != black_player_username),
