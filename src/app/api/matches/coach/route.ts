...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sessionData.userType !== UserType.COACH) {
      return NextResponse.json(
        { success: false, message: 'Only coaches can access coach matches' },
        { status: 403 }
      );
    }
    ...
    // Get all matches for teams coached by this coach
    const [matches] = await db.query(`
      ...
      WHERE t1.coach_username = ? OR t2.coach_username = ?
    ...
    console.error('Get coach matches error:', error);
    return NextResponse.json(
      { 
        success: false, 
        message: 'An error occurred while fetching coach matches' 
      },
      { status: 500 }
    );
  }
