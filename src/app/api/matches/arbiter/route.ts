...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sessionData.userType !== UserType.ARBITER) {
      return NextResponse.json(
        { success: false, message: 'Only arbiters can access arbiter matches' },
        { status: 403 }
      );
    }
    ...
    // Get all matches officiated by this arbiter
    ...
    console.error('Get arbiter matches error:', error);
    return NextResponse.json(
      { 
        success: false, 
        message: 'An error occurred while fetching arbiter matches' 
      },
      { status: 500 }
    );
  }
